extends Node
## Central game state manager. Handles run state, stats, and game flow.
...
# Player stats (base values, modified by traits)
...
# Difficulty scaling
const ENEMY_HP_SCALE := 1.25
const ENEMY_DMG_SCALE := 1.15
const ENEMY_COUNT_SCALE := 1.3
...
func get_wave_enemy_count(wave: int) -> int:
	var base := 8
	return int(base * pow(ENEMY_COUNT_SCALE, wave - 1))

func get_wave_enemy_hp_mult(wave: int) -> float:
	return pow(ENEMY_HP_SCALE, wave - 1)

func get_wave_enemy_dmg_mult(wave: int) -> float:
	return pow(ENEMY_DMG_SCALE, wave - 1)

func get_wave_enemy_speed_mult(wave: int) -> float:
	return pow(ENEMY_SPEED_SCALE, wave - 1)
...
func _on_enemy_killed(enemy: Node3D, position: Vector3) -> void:
	...
	if enemies_alive <= 0 and state == GameState.PLAYING:
		if is_boss_wave:
			return  # Handled by boss_killed
		EventBus.wave_completed.emit(current_wave)

func _on_exp_collected(amount: float) -> void:
	var modified_amount := amount * player_traits.get("exp_mult", 1.0)
